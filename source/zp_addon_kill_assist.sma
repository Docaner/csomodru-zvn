...
new g_pCvarAssistHp, g_SyncMessage
...
public plugin_cfg()
{
	...
	g_pCvars[CVAR_MONEY] = register_cvar("aka_money", "1000")
	g_pCvars[CVAR_EXP] = register_cvar("aka_exp", "1")
	g_pCvars[CVAR_DAMAGE] = register_cvar("aka_damage", "60.0")
	g_pCvars[CVAR_ALGORITHM] = register_cvar("aka_algorithm", "1")
	g_pCvars[CVAR_CHATMESSAGE] = register_cvar("aka_chatmessage", "0")
...
public CBasePlayer_Killed_Pre(iVictim, iKiller)
{
	...
	if(bIsAssistantConnected)
	{
		...
		{
			if(iAddMoney > 0)
				zp_set_user_money(iAssistant, zp_get_user_money(iAssistant) + iAddMoney)
			
			if(iAddExp > 0)
				zpe_set_user_exp(iAssistant, zpe_get_user_exp(iAssistant) + iAddExp)
			...
			set_hudmessage(200, 200, 200, -1.0, 0.32, 2, 2.0, 1.0, 0.06, 2.0, -1);
			ShowSyncHudMsg(iAssistant, g_SyncMessage, "+1 EXP kill assisted by %s", g_ePlayerData[iKiller][NAME])
		}
