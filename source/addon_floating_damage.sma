...
new const EntityDamagerModel[ ] =			"models/zp_br_cso/floating_damage.mdl";
...
public plugin_init( )
{
	...
	RegisterHam( Ham_TraceAttack, "info_target", "NPC__TraceAttack_Post", true );
	RegisterHam( Ham_TakeDamage, "info_target", "NPC__TakeDamage_Post", true );
#else
	/* -> HamSandwich: Player <- */
...
new Float:g_vecOriginAttack[33][3];
new g_iAttackerLastHit[33];

public NPC__TraceAttack_Post(pVictim, pAttacker, Float:flDamage, Float:vecDir[3], pTrace, iBitDamage)
{
	if(!is_user_alive( pAttacker ) || !UserDamagerEnabled( pAttacker ))
		return;
	
	if(!IsAliveNPC(pVictim))
		return;

	g_iAttackerLastHit[pAttacker] = get_tr2(pTrace, TR_iHitgroup);

	get_tr2(pTrace, TR_vecEndPos, g_vecOriginAttack[pAttacker]);
	vec_scale_to_player(pAttacker, g_vecOriginAttack[pAttacker], g_vecOriginAttack[pAttacker])
}

public NPC__TakeDamage_Post(const pVictim, const pInflictor, const pAttacker, const Float: flDamage, iBitDamage)
{
	if(!is_user_alive( pAttacker ) || !UserDamagerEnabled( pAttacker ))
		return;
	...
	gl_flUserTotalDamage[ pAttacker ][ 0 ] += flDamage;

	if(iBitDamage & DMG_GRENADE)
	{
		get_entvar(pVictim, var_origin, g_vecOriginAttack[pAttacker]);
		vec_scale_to_player(pAttacker, g_vecOriginAttack[pAttacker], g_vecOriginAttack[pAttacker]);
	}

	CDamager__SpawnEntity( pAttacker, pVictim, g_vecOriginAttack[pAttacker], gl_flUserTotalDamage[ pAttacker ][ 0 ], bool: ( g_iAttackerLastHit[pAttacker] == HIT_HEAD ) );
}

stock vec_scale_to_player(pPlayer, Float:vecIn[], Float:vecOut[])
{
	new Float:vecAttacker[3]; get_entvar(pPlayer, var_origin, vecAttacker);
	new Float:vecDir[3]; xs_vec_sub(vecAttacker, vecIn, vecDir);
	xs_vec_normalize(vecDir, vecDir);
	xs_vec_mul_scalar(vecDir, 100.0, vecDir);
	xs_vec_add(vecIn, vecDir, vecOut);
}
...
public CDamager__SpawnEntity( const pPlayer, const pVictim, Vector3( vecOrigin ), const Float: flDamage, const bool: bHeadShot )
{
	...
	if ( !is_nullent( pEntity ) )
	{
		if(IsUserValid(pVictim))
			get_view_pos(pVictim, pPlayer, vecOrigin);
	...
	// Cache vars
	//set_entvar( pEntity, var_start_origin, vecAdd );
...
stock get_view_pos(pVictim, pAttacker, Float:vecSpawn[])
{
	new Float:vecAttacker[3]; get_entvar(pAttacker, var_origin, vecAttacker);
	new Float:vecVictim[3]; get_entvar(pVictim, var_origin, vecVictim);

	new Float:vecDir[3]; 
	xs_vec_sub(vecAttacker, vecVictim, vecDir);
	xs_vec_normalize(vecDir, vecDir);

	new Float:vecViewOfs[3]; get_entvar(pVictim, var_view_ofs, vecViewOfs);
	vecViewOfs[2] += 30.0;
	xs_vec_add(vecVictim, vecViewOfs, vecSpawn);

	xs_vec_add_scaled(vecSpawn, vecDir, 20.0, vecSpawn);
}

public CDamager__FindActiveEntity( const pPlayer, const pVictim )
{
	if ( IsUserValid( pVictim ) && gl_flUserTotalDamage[ pPlayer ][ pVictim ] <= 0.0 || IsNPC( pVictim ) && gl_flUserTotalDamage[ pPlayer ][ 0 ] <= 0.0 )
		return NULLENT;
...
public CDamager__Think( const pEntity )
{
	...
	if ( ( flRenderAmt -= 15.0 ) && flRenderAmt <= 15.0 )
	{
		...
		if ( IsUserValid( pVictim ) )
			...
			gl_flUserTotalDamage[ pOwner ][ 0 ] = 0.0;
