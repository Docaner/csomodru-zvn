...
#define WEAPON_ANIM_SLASH 2
#define WEAPON_ANIM_RELOAD 3
#define WEAPON_ANIM_DRAW 4

// From model: Frames/FPS
#define WEAPON_ANIM_IDLE_TIME 121/15.0
#define WEAPON_ANIM_SHOOT_TIME 31/30.0
#define WEAPON_ANIM_SLASH_TIME 53/30.0
#define WEAPON_ANIM_RELOAD_TIME 81/30.0
#define WEAPON_ANIM_DRAW_TIME 31/30.0

#define WEAPON_SPECIAL_CODE 2253
#define WEAPON_CSW CSW_M3
#define WEAPON_REFERENCE "weapon_m3"
#define WEAPON_NEW_NAME "x/weapon_sgdrill"
#define WEAPON_HUD "sprites/x/640hud140.spr"
#define WEAPON_HUD_AMMO "sprites/x/640hud17.spr"

#define WEAPON_ANIM_EXTENSION "m249"
#define WEAPON_ANIM_EXTENSION_SLASH "knife"

#define WEAPON_ITEM_NAME "Magnum Drill"
#define WEAPON_ITEM_COST 0

#define WEAPON_MODEL_VIEW "models/x/v_sgdrill.mdl" 
#define WEAPON_MODEL_PLAYER "models/x/p_sgdrill.mdl" 
#define WEAPON_MODEL_WORLD "models/x/w_sgdrill.mdl"
#define WEAPON_SOUND_SHOOT "weapons/sgdrill-1.wav"
#define WEAPON_SOUND_SLASH "weapons/sgdrill_slash.wav"
#define WEAPON_BODY 0

#define MAGNUM_DRILL_DISTANCE 75.0
#define MAGNUM_DRILL_DAMAGE_SLASH random_float(250.0, 450.0)
#define MAGNUM_DRILL_KNOCKBACK 175.0

#define WEAPON_AMMO_COST 250
#define WEAPON_MAX_CLIP 25
#define WEAPON_DEFAULT_AMMO 105
#define WEAPON_RATE 0.3
#define WEAPON_PUNCHAGNLE 0.8
...
public plugin_init()
{
	register_plugin("[X-CSO] Weapon: Mangum Drill", "1.1", "xUnicorn (t3rkecorejz) / Phenix / D34");
...
public plugin_precache()
{
	...
	engfunc(EngFunc_PrecacheSound, WEAPON_SOUND_SLASH);
...
// [ Amxx ]
public plugin_natives() register_native("zp_give_user_magnum", "Command_GiveWeapon", 1);
...
public CWeapon__Deploy_Post(iItem)
{
	...
	UTIL_SendWeaponAnim(iPlayer, WEAPON_ANIM_DRAW, 0);
...
public CWeapon__PostFrame_Pre(iItem)
{
	...
	if(get_pdata_int(iItem, m_iWeaponState, linux_diff_weapon) == 0)
	{
		...
		if(iButton & IN_ATTACK2 && !get_pdata_int(iItem, m_fInReload, linux_diff_weapon) && get_pdata_float(iItem, m_flNextSecondaryAttack, linux_diff_weapon) <= 0.0)
		{
			...
			formatex(szAnimation, charsmax(szAnimation), pev(iPlayer, pev_flags) & FL_DUCKING ? "crouch_shoot_%s" : "ref_shoot_%s", WEAPON_ANIM_EXTENSION_SLASH);
			UTIL_PlayerAnimation(iPlayer, szAnimation);

			UTIL_SendWeaponAnim(iPlayer, WEAPON_ANIM_SLASH, 0);
			emit_sound(iPlayer, CHAN_WEAPON, WEAPON_SOUND_SLASH, VOL_NORM, ATTN_NORM, 0, PITCH_NORM);

			set_pdata_int(iItem, m_iWeaponState, 1, linux_diff_weapon);
			set_pdata_float(iItem, m_flNextPrimaryAttack, WEAPON_ANIM_SLASH_TIME, linux_diff_weapon);
			set_pdata_float(iItem, m_flNextSecondaryAttack, WEAPON_ANIM_SLASH_TIME, linux_diff_weapon);
			set_pdata_float(iItem, m_flTimeWeaponIdle, WEAPON_ANIM_SLASH_TIME, linux_diff_weapon);
			set_pdata_float(iPlayer, m_flNextAttack, 18/30.0, linux_diff_player);
	...
	else
	{
		...
		new Float: flSendAngles[] = { 0.0, -10.0, 10.0, -5.0, 5.0, -5.0, 5.0, 0.0, 0.0, -20.0, 20.0, -10.0, 10.0, -10.0, 10.0, 0.0, 0.0 }
		new Float: flSendAnglesUp[] = { 0.0, 0.0, 0.0, 7.5, 7.5, -7.5, -7.5, -15.0, 15.0, 0.0, 0.0, 15.0, 15.0, -15.0, -15.0, -30.0, 30.0 }
		new Float: flTan;
		new Float: flMul;
		...
		for(new i; i < sizeof flSendAngles; i++)
		{
			...
			flEnd[0] = (flForw[0] * MAGNUM_DRILL_DISTANCE) + (flRight[0] * flTan * MAGNUM_DRILL_DISTANCE) + flUp[0] * flSendAnglesUp[i];
			flEnd[1] = (flForw[1] * MAGNUM_DRILL_DISTANCE) + (flRight[1] * flTan * MAGNUM_DRILL_DISTANCE) + flUp[1] * flSendAnglesUp[i];
			flEnd[2] = (flForw[2] * MAGNUM_DRILL_DISTANCE) + (flRight[2] * flTan * MAGNUM_DRILL_DISTANCE) + flUp[2] * flSendAnglesUp[i];
				
			flMul = (MAGNUM_DRILL_DISTANCE/vector_length(flEnd));
			flEnd[0] *= flMul;
			flEnd[1] *= flMul;
			flEnd[2] *= flMul;
			...
			if(pHit > 0 && pHitEntity != pHit)
			{
				...
				{
					ExecuteHamB(Ham_TakeDamage, pHit, iPlayer, iPlayer, MAGNUM_DRILL_DAMAGE_SLASH, DMG_NEVERGIB | DMG_CLUB);
				}
				else
				{
					FakeTraceAttack(pHit, iPlayer, MAGNUM_DRILL_DAMAGE_SLASH, flForw, iTrace, DMG_NEVERGIB | DMG_CLUB);
					FakeKnockBack(pHit, flForw, MAGNUM_DRILL_KNOCKBACK);
				}
...
public CWeapon__Reload_Pre(iItem)
{
	...
	UTIL_SendWeaponAnim(iPlayer, WEAPON_ANIM_RELOAD, 0);
...
public CWeapon__WeaponIdle_Pre(iItem)
{
	...
	UTIL_SendWeaponAnim(iPlayer, WEAPON_ANIM_IDLE, 0);
...
public CWeapon__PrimaryAttack_Pre(iItem)
{
	...
	UTIL_SendWeaponAnim(iPlayer, WEAPON_ANIM_SHOOT, 0);
