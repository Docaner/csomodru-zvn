...
#define PLUGIN "[KZ] Weapon: Quad Barrel"
#define VERSION "1.0"
...
#define ANIM_IDLE_TIME 6.1
#define ANIM_SHOOT_TIME 1.36
#define ANIM_RELOAD_TIME 3.03
#define ANIM_DRAW_TIME 1.1

#define WEAPON_KEY 1028
#define WEAPON_CSW CSW_XM1014
#define WEAPON_OLD "weapon_xm1014"
#define WEAPON_NEW "zp_br_cso/weapons2/weapon_qbarrel"
...
#define WEAPON_NAME "QBarrel"
#define WEAPON_COST 0

#define WEAPON_MODEL_V "models/zp_br_cso/weapons2/v_qbarrel.mdl"

#define WEAPON_MODEL_W "models/zp_br_cso/other/w_weapons_b1.mdl"
#define WEAPON_SOUND_S "weapons/qbarrel-1.wav"
#define WEAPON_BODY 9

#define WEAPON_CLIP 4
#define WEAPON_AMMO 32
#define WEAPON_RATE 0.37
#define WEAPON_RECOIL 0.75
...
public plugin_natives() register_native("zp_give_user_qbarrel", "give_weapon", 1);
...
public fw_Item_PostFrame(iItem) 
{
	...
	if(get_pdata_int(iItem, m_fInSpecialReload, 4) == 2) 
	{
	...
	else if(iButton & IN_ATTACK2 && iClip) 
	{
...
public fw_Weapon_Reload(iItem) {
	...
	set_pdata_int(iItem, m_fInSpecialReload, 2, 4);
...
public fw_Weapon_PrimaryAttack(iItem) 
{
	...
	if(get_pdata_int(iItem, m_iClip, 4) == 0) {
		...
		set_pdata_float(iItem, m_flNextPrimaryAttack, 0.1, 4);
		return HAM_SUPERCEDE;
	}
...
{
	if(!CustomItem(iItem)) return HAM_IGNORED;
	static i, iClip; iClip = get_pdata_int(iItem, m_iClip, 4);
	...
	fm_ham_hook(true);
	state FireBullets: Enabled;
	for(i = 0; i < iClip;i++) ExecuteHam(Ham_Weapon_PrimaryAttack, iItem);
	state FireBullets: Disabled;
	unregister_forward(FM_TraceLine, fw_TraceLine, 1);
	fm_ham_hook(false);
	static iPlayer; iPlayer = get_pdata_cbase(iItem, m_pPlayer, 4);
	...
	set_pdata_float(iItem, m_flNextPrimaryAttack, ANIM_SHOOT_TIME, 4);
	set_pdata_float(iItem, m_flNextSecondaryAttack, ANIM_SHOOT_TIME, 4);
	set_pdata_float(iItem, m_flTimeWeaponIdle, ANIM_SHOOT_TIME, 4);
