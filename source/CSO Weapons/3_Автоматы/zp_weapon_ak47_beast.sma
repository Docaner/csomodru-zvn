...
#define WEAPON_STAB_KNIFE_DISTANCE 107.25
#define WEAPON_STAB_KNIFE_DAMAGE random_float(300.0, 425.0)
#define WEAPON_STAB_KNIFE_KNOCKBACK 300.0

#define WEAPON_MAX_CLIP 30
#define WEAPON_DEFAULT_AMMO 90
#define WEAPON_RATE 0.102
#define WEAPON_PUNCHAGNLE 0.725
...
public plugin_init()
{
	register_plugin("[X-CSO] Weapon: AK47 Beast", "1.0", "xUnicorn (t3rkecorejz) / Batcoh: Code base");
...
public CWeapon__Deploy_Post(iItem)
{
	...
	set_pdata_float(iItem, m_flLastEventCheck, get_gametime() + 0.2, linux_diff_weapon);
	...
	else
	{
		...
		for(new i; i < sizeof flSendAngles; i++)
		{
			...
			if(pHit > 0 && pHitEntity != pHit)
			{
				...
				else if(!is_user_block_dmg(pHit))
				{
