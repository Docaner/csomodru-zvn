...
#define PLUGIN 					"[ZP] Extra: CSO Weapon Charger 5"
#define VERSION 				"1.1"
...
// Main
#define WEAPON_KEY					1046
#define WEAPON_NAME 				"zp_br_cso/weapons4/weapon_charger5_b1"

#define WEAPON_REFERANCE			"weapon_ump45"
...
#define WEAPON_TIME_NEXT_ATTACK_B 		0.9
#define WEAPON_TIME_DELAY_DEPLOY 		1.03
#define WEAPON_TIME_DELAY_RELOAD 		3.8

#define WEAPON_DAMAGE  	  			1.33
#define WEAPON_DAMAGE_B				random_float(400.0, 600.0)

#define ZP_ITEM_NAME				"Charger-5" 
#define ZP_ITEM_COST				0

// Models
#define MODEL_WORLD					"models/zp_br_cso/other/w_weapons4.mdl"
#define MODEL_WORLD_BODY 			3
#define MODEL_VIEW					"models/zp_br_cso/weapons4/v_charger5_b1.mdl"

// Sounds
#define SOUND_FIRE					"weapons/charger5-1.wav"
#define SOUND_FIRE_B				"weapons/charger7-2.wav"

// Sprites
#define WEAPON_HUD_TXT				"sprites/zp_br_cso/weapons4/weapon_charger5_b1.txt"
#define WEAPON_HUD_SPR_1			"sprites/zp_br_cso/weapons4/hud/640hud167.spr"
#define WEAPON_HUD_SPR_2			"sprites/zp_br_cso/weapons4/hud/640hud3.spr"
...
Weapon_OnPrimaryAttack(const iItem, const iPlayer, const iClip, const iAmmoPrimary)
{
	...
	Punchangle(iPlayer, .iVecx = -2.0, .iVecy = random_float(1.09, -1.09), .iVecz = 0.0);
	...
	if(get_pdata_int(iItem, m_iWeaponState, 4) < 5) 
	{
		...
		if(!(get_pdata_int(iItem, m_iShotsFired, 4) % 40)) 
		{
			Update_StatusIcon(iItem, iPlayer, 0);
...
LaserAttack(const iItem, const iPlayer)
{
	...
	static Float:StartOrigin[3];GetWeaponPosition(iPlayer, 15.0, 5.0, -4.0, StartOrigin);
	static Float:iVecx;
	static Float:iVecy;	
	static iTrace;
	...
	do{iVecx = random_float(-0.3, 0.3) + random_float(-0.3, 0.3);iVecy = random_float(-0.3, 0.3) + random_float(-0.3, 0.3);}
	...
	CreateBeam(StartOrigin, vecEnd, iBlood[2], 2, 10);
	CreateBeam(StartOrigin, vecEnd, iBlood[2], 3, 10);
	CreateBeam(StartOrigin, vecEnd, iBlood[2], 4, 10);
	CreateBeam(StartOrigin, vecEnd, iBlood[2], 5, 10);
...
CreateBeam(const Float:vStart[3], const Float:vEnd[3], const iModel, const iLife, const iWigth)
{
	MESSAGE_BEGIN(MSG_BROADCAST, SVC_TEMPENTITY, {0.0,0.0,0.0}, 0);
