...
#define PLUGIN "[KZ] Weapon: HK XM8"
#define VERSION "1.0"
...
#define ANIM_IDLE_TIME 0.134
#define ANIM_SHOOT_TIME 1.94
#define ANIM_RELOAD_A_TIME 3.5
#define ANIM_RELOAD_B_TIME 3.5
#define ANIM_DRAW_A_TIME 1.27
#define ANIM_DRAW_B_TIME 1.27
#define ANIM_CHANGE_TIME 6.04

#define WEAPON_KEY 1037
#define WEAPON_CSW CSW_GALIL
#define WEAPON_OLD "weapon_galil"
#define WEAPON_NEW "zp_br_cso/weapons2/weapon_xm81"
#define WEAPON_NEW_EX "zp_br_cso/weapons2/weapon_xm82_b1"
#define WEAPON_HUD "sprites/zp_br_cso/weapons2/hud2/640hud2.spr"
...
#define WEAPON_NAME "HK XM8"
#define WEAPON_COST 0

#define WEAPON_MODEL_V "models/zp_br_cso/weapons2/v_xm8.mdl"

#define WEAPON_MODEL_W "models/zp_br_cso/other/w_weapons_b1.mdl"
#define WEAPON_SOUND_S "weapons/xm8-1.wav"
#define WEAPON_SOUND_S2 "weapons/xm8-2.wav"
#define WEAPON_BODY 21

#define WEAPON_CLIP 30
#define WEAPON_AMMO 90

#define WEAPON_RATE_A 0.102
#define WEAPON_RATE_A_EX 0.13
#define WEAPON_RECOIL_A 0.94
#define WEAPON_DAMAGE_A 1.44

#define WEAPON_RATE_B 0.173
#define WEAPON_RECOIL_B 0.87
#define WEAPON_DAMAGE_B 1.56
...
public plugin_init() {
	...
	RegisterHam(Ham_CS_Item_GetMaxSpeed, WEAPON_OLD, "fw_Item_GetMaxSpeed");
	...
	g_iMsgID_HideWeapon = get_user_msgid("HideWeapon");
...
public fw_Item_GetMaxSpeed(iItem) {
	...
	if(Get_WeaponState(iItem)) UTIL_UpdateHideWeapon(iPlayer, get_pdata_int(iPlayer, m_iHideHUD, 5) | HIDEHUD_CROSSHAIR);
	else UTIL_UpdateHideWeapon(iPlayer, get_pdata_int(iPlayer, m_iHideHUD, 5) & ~HIDEHUD_CROSSHAIR);
}
public fw_Item_Holster_Post(iItem) {
	...
	UTIL_UpdateHideWeapon(iPlayer, get_pdata_int(iPlayer, m_iHideHUD, 5) & ~HIDEHUD_CROSSHAIR);
	return HAM_IGNORED;
}
...
public fw_Item_PostFrame(iItem) {
	...
	if(get_pdata_float(iItem, m_flNextSecondaryAttack, 4) <= 0.0) {
		if(iButton & IN_ATTACK2) {
			if(Get_WeaponState(iItem)) set_pdata_int(iPlayer, m_iFOV, get_pdata_int(iPlayer, m_iFOV, 5) == 90 ? 40 : 90);
			else set_pdata_int(iPlayer, m_iFOV, get_pdata_int(iPlayer, m_iFOV, 5) == 90 ? 75 : 90);
...
public fw_Weapon_PrimaryAttack(iItem) {
	...
	static Float: flRate;
	if(Get_WeaponState(iItem)) flRate = WEAPON_RATE_B;
	else flRate = get_pdata_int(iPlayer, m_iFOV, 5) != 90 ? WEAPON_RATE_A_EX : WEAPON_RATE_A;

	set_pdata_float(iItem, m_flNextPrimaryAttack, flRate, 4);
...
public UTIL_UpdateHideWeapon(iPlayer, iFlags) {
	message_begin(MSG_ONE, g_iMsgID_HideWeapon, _, iPlayer);
	write_byte(iFlags);
	message_end();

	set_pdata_int(iPlayer, m_iHideHUD, iFlags, 5);
	set_pdata_int(iPlayer, m_iClientHideHUD, iFlags, 5);
}
