...
#define PLUGIN "[ZMO]Weapon: Skull-4"
#define VERSION "1.0"
...
#define ANIM_DRAW 2
#define ANIM_ATTACK_LEFT 3
#define ANIM_ATTACK_RIGHT 4

#define ANIM_IDLE_TIME 2.04
#define ANIM_RELOAD_TIME 3.44
#define ANIM_DRAW_TIME 1.24

#define WEAPON_KEY 1014
#define WEAPON_OLD "weapon_aug"
#define WEAPON_NEW "zp_br_cso/weapons2/weapon_skull4"

#define WEAPON_MODEL_V "models/zp_br_cso/weapons2/v_skull4.mdl"
#define WEAPON_MODEL_P "models/zp_br_cso/weapons2/p_skull4.mdl"
#define WEAPON_MODEL_W "models/zp_br_cso/other/weapons_models_all.mdl"
#define WEAPON_SOUND_S "weapons/skull4_shoot1.wav"
#define WEAPON_BODY 20

#define WEAPON_CLIP 40
#define WEAPON_AMMO 90
#define WEAPON_RATE 0.2
#define WEAPON_RECOIL 1.3
...
public plugin_init() {
	...
	g_ZP_Item = zp_register_extra_item("Skull-4", 0, ZP_TEAM_HUMAN);
}
...
public fw_Item_Deploy_Post(iItem) {
	...
	set_pdata_string(iPlayer, m_szAnimExtention * 4, "dualpistols", -1, 20);
...
public fw_Weapon_PrimaryAttack(iItem) {
	...
	if(get_pdata_int(iItem, m_iWeaponState, 4) == 0 ) 
	{
		set_pdata_int(iItem, m_iWeaponState, 1, 4);
		UTIL_SendWeaponAnim(iPlayer, ANIM_ATTACK_LEFT);

		static szAnimation[32]; 
		format(szAnimation, charsmax(szAnimation), pev(iPlayer, pev_flags) & FL_DUCKING ? "crouch_shoot2_dualpistols" : "ref_shoot2_dualpistols");
		SetAnimation(iPlayer, szAnimation);
	}
	else {
		set_pdata_int(iItem, m_iWeaponState, 0, 4);
		UTIL_SendWeaponAnim(iPlayer, ANIM_ATTACK_RIGHT);

		static szAnimation[32]; 
		format(szAnimation, charsmax(szAnimation), pev(iPlayer, pev_flags) & FL_DUCKING ? "crouch_shoot_dualpistols" : "ref_shoot_dualpistols");
		SetAnimation(iPlayer, szAnimation);
	}
...
stock SetAnimation( const iPlayer, const szAnim[] )
{
	...
	set_pdata_int( iPlayer, 40, bLoops, 4 );
	set_pdata_int( iPlayer, 39, 0, 4 );

	set_pdata_float( iPlayer, 36, flFrameRate, 4 );
	set_pdata_float( iPlayer, 37, flGroundSpeed, 4 );
	set_pdata_float( iPlayer, 38, flGameTime, 4 );

	set_pdata_int( iPlayer, 73, 28, 5 );
	set_pdata_int( iPlayer, 74, 28, 5 );
	set_pdata_float( iPlayer, 220, flGameTime, 5 );
}