...
#define model_w					"models/zmo/w_models_b1.mdl"
...
public plugin_init(){
	...
	g_iItemID = zp_register_extra_item("Deagle Crimson Hunter", 0, ZP_TEAM_HUMAN);
...
public buy_secondary_ammo(iPlayer) {
	...
	if(!Is_CustomItem(iItem)) return;

	// SetAmmoValue(iPlayer, get_pcvar_num(Cvar[3]));
	// SetAmmoMax(iPlayer, get_pcvar_num(Cvar[4]));
	// SetAmmoCost(iPlayer, 100);
...
public fw_Weapon_Deploy_Post(ent){
	if(pev_valid(ent) != 2 || !Is_CustomItem(ent))return HAM_IGNORED;
...
public fw_TakeDamage(victim,inflictor,attacker,Float:damage)<FireBullets:Enabled>{ 
	if(pev_valid(attacker) != 2) return HAM_IGNORED;
