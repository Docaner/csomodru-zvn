...
public CGrenade__Think(iEntity) {
	...
	if(pev(iEntity, pev_flTimeStepSound) == WEAPON_SPECIAL_CODE) {
		...
		{
			new Float: flDistance, Float: vecVicOrigin[3], Float: vecVelocity[3];
			...
			for(new iVictim = 1; iVictim <= MaxClients; iVictim++)
			{
				...
				if(zp_get_user_zombie(iVictim))
				{
					...
					UTIL_GetSpeedVectorTo2Point(vecOrigin, vecVicOrigin, GRENADE_KNOCKBACK + (1.0 - (flDistance / GRENADE_RADIUS_KNOCK)), vecVelocity);
				}
				else
				{
					...
					UTIL_GetSpeedVectorTo2Point(vecOrigin, vecVicOrigin, GRENADE_KNOCKBACK_HUMAN + (1.0 - (flDistance / GRENADE_RADIUS_KNOCK_HUMAN)), vecVelocity);
				}
...
stock UTIL_GetSpeedVectorTo2Point(const Float:vecOrigin1[3], const Float:vecOrigin2[3], Float:flSpeed, Float:vecVelocity[3]) {
