...
/* ■■■■■■■■■■■■■■■■■■■■■■■■■■■■ CONFIG START ■■■■■■■■■■■■■■■■■■■■■■■■■■■■ */
#define MENU_NUBER_COLOR "\r" // цвет нумерации меню: \w - белый, \y - желтый, \r - красный, \d - серый
...
stock generate_dictionary() {
	...
	if(!file_exists(cfg_file)) {
		...
		write_file(cfg_file, "MM_MENU_TITLE = \wМеню мута");
		write_file(cfg_file, "MM_MENU_ITEM_ALL = Заткнуть всех");
		write_file(cfg_file, "MM_MENU_LABEL_GAGGED = \r[в муте]");
		write_file(cfg_file, "MM_ALERT_OPTION_SET = заткнули");
		write_file(cfg_file, "MM_ALERT_OPTION_UNSET = сняли мут с");
		write_file(cfg_file, "MM_ALERT_CHAT = ^^4[MUTE] ^^1Вы %L ^^3%n");
		write_file(cfg_file, "MM_ALERT_CHAT_ALL_SET = ^^4[MUTE] ^^1Вы заткнули ^^3всех");
		write_file(cfg_file, "MM_ALERT_CHAT_ALL_UNSET = ^^4[MUTE] ^^1Вы сняли мут ^^3со всех");
		write_file(cfg_file, "^n[en]");
		write_file(cfg_file, "MM_MENU_TITLE =\wMenu mute");
		write_file(cfg_file, "MM_MENU_ITEM_ALL = Mute all");
		write_file(cfg_file, "MM_MENU_LABEL_GAGGED = \r[muted]");
		write_file(cfg_file, "MM_ALERT_OPTION_SET = muted");
		write_file(cfg_file, "MM_ALERT_OPTION_UNSET = unmuted");
		write_file(cfg_file, "MM_ALERT_CHAT = ^^4[MUTE] ^^1You %L ^^3%n");
		write_file(cfg_file, "MM_ALERT_CHAT_ALL_SET = ^^4[MUTE] ^^1You muted ^^3all");
		write_file(cfg_file, "MM_ALERT_CHAT_ALL_UNSET = ^^4[MUTE] ^^1You unmuted ^^3all");
	}
}