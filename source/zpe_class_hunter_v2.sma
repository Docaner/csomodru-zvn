...
#define ZM_CLASS_NAME 			"ML_HUNTER_NAME"
#define ZM_CLASS_INFO 			"ML_HUNTER_INFO"
#define ZM_CLASS_MODEL 			"zp_br_hunter_b10"
#define ZM_CLASS_CLAW 			"claws_hunter.mdl"
#define ZM_CLASS_HEALTH 		1750
#define ZM_CLASS_SPEED 			245
#define ZM_CLASS_GRAVITY		0.90
#define ZM_CLASS_KNOCKBACK		0.52
...
public plugin_precache()
{
	...
	g_iZClassID = zp_register_zombie_class(ZM_CLASS_NAME, ZM_CLASS_INFO, ZM_CLASS_MODEL, ZM_CLASS_CLAW, ZM_CLASS_HEALTH, ZM_CLASS_SPEED, ZM_CLASS_GRAVITY, ZM_CLASS_KNOCKBACK);
}
...
{
	if(zp_get_user_zombie(iPlayer) && zp_get_user_zombie_class(iPlayer) == g_iZClassID && !zp_get_user_nemesis(iPlayer))
	{
...
public EV_RoundStart()
{
	...
	public CWeapon__Deploy_Post(iItem)
	{
		...
		if(zp_get_user_zombie_class(iPlayer) != g_iZClassID) return HAM_IGNORED;
...
public Command_Ability(iPlayer)
{
	...
	if(zp_get_user_zombie_class(iPlayer) == g_iZClassID)
	{	
...
{
	if(!zp_get_user_zombie(pVictim) || zp_get_user_zombie_class(pVictim) != g_iZClassID)
		return;
...
ResetStats(iPlayer)
{
	if(!zp_get_user_zombie(iPlayer) || zp_get_user_zombie_class(iPlayer) != g_iZClassID)
		return;
