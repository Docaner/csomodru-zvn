...
#define KNOCK_MUL 1.25 // Можитель, если жертва стоит
...
#define KNOCK_MUL_BACK 0.25 // Множитель, если жертва повернута спиной
#define KNCOK_MUL_NEMESIS 0.05 // Множитель, если жертва - немезида
...
new Float:g_flKnockBase = KNOCK_BASE;
new Float:g_flMulDuck = KNOCK_MUL_DUCK;
...
public plugin_natives()
{
	...
	register_native("zp_set_base_knock", "@zp_set_base_knock", 1);
	register_native("zp_get_base_knock", "@zp_get_base_knock", 1);

	register_native("zp_set_mul_duck_knock", "@zp_set_mul_duck_knock", 1);
	register_native("zp_get_mul_duck_knock", "@zp_get_mul_duck_knock", 1);
}
...
*/
bool:@zp_set_user_velocitymifier(iPlayer, Float:flModifier)
{
 ...
 * @noreturn
 */
@zp_set_user_block_velocitymifier(const id, const iValue)
	if(iValue) 
 ...
 */
@zp_get_user_block_velocitymifier(const id)
 ...
 */
bool:@zp_set_user_knock_by_trace(iVictim, iAttacker, pTrace, Float:flRadius, Float:flMulKnock)
{
 ...
 * @return 					- вернёт true, если кнокбек будет нанесён жертве 
 */
bool:@zp_set_user_knock_by_missile(iVictim, iAttacker, Float:flRadius, Float:flMulKnock)
{
 ...
 * @noreturn
 */
@zp_set_user_mul_knock(id, Float:flValue)
 ...
 */
Float:@zp_get_user_mul_knock(id)
	return g_flUserKnocks[id];

@zp_set_base_knock(Float:flKnock)
	g_flKnockBase = flKnock

Float:@zp_get_base_knock() 
	return g_flKnockBase;

@zp_set_mul_duck_knock(Float:flKnock)
	g_flMulDuck = flKnock

Float:@zp_get_mul_duck_knock() 
	return g_flMulDuck;
...
stock set_user_knock(iVictim, iAttacker, Float:flMulKnock, Float:vecDir[3])
{
	...
	new Float:flKnock = g_flKnockBase;
