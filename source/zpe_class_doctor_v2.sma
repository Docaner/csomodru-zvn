...
#define ZM_CLASS_NAME		"ML_HEALER_NAME"
#define ZM_CLASS_INFO		"ML_HEALER_INFO"
#define ZM_CLASS_MODEL		"zp_br_healer_b10"
#define ZM_CLASS_CLAW		"claws_healer_b6.mdl"
#define ZM_CLASS_HEALTH		2350
#define ZM_CLASS_SPEED		240
#define ZM_CLASS_GRAVITY	0.92
#define ZM_CLASS_KNOCK		0.50
...
public plugin_precache( )
{
	...
	g_iZClassHeal = zp_register_zombie_class( ZM_CLASS_NAME, ZM_CLASS_INFO, ZM_CLASS_MODEL, ZM_CLASS_CLAW, ZM_CLASS_HEALTH, ZM_CLASS_SPEED, ZM_CLASS_GRAVITY, ZM_CLASS_KNOCK );
...
{
	if(zp_get_user_zombie(iPlayer) && zp_get_user_zombie_class(iPlayer) == g_iZClassHeal && !zp_get_user_nemesis(iPlayer))
	{
...
public EV_RoundStart()
{
	...
	public CWeapon__Deploy_Post(iItem)
	{
		...
		if(zp_get_user_zombie_class(iPlayer) != g_iZClassHeal) return HAM_IGNORED;
...
public Command_Ability( iPlayer )
{
	...
	if(zp_get_user_zombie_class(iPlayer) == g_iZClassHeal)
	{	
		...
		else
		{
			if(g_flAbilityWait[iPlayer] <= flGameTime)
			{
				...
				while( ( iVictim = engfunc( EngFunc_FindEntityInSphere, iVictim, vecOrigin, 225.0 ) ) )
				{
					...
					if( flCurrentHealth < flMaximumHealth )
					{
						...
						flSetHealth = floatmin( flMaximumHealth, ( flCurrentHealth + 650 ) );
						...
						zp_set_user_money( iPlayer, zp_get_user_money( iPlayer ) + 500 )
					}
				}
				...
				engfunc( EngFunc_WriteCoord, vecOrigin [ 0 ] );
				engfunc( EngFunc_WriteCoord, vecOrigin [ 1 ] );
				engfunc( EngFunc_WriteCoord, vecOrigin [ 2 ] + 225.0 );
...
public FM_Hook_PlayerPreThink_Pre(iPlayer)
{
	...
	if(zp_get_user_zombie_class(iPlayer) != g_iZClassHeal) return FMRES_IGNORED;
