...
new const Resource[][] = {
	"models/npc_hardcs/npc_oberon.mdl",				// 0
...
public Boss_Spawn(Float:hp, Ent) {
	...
	engfunc(EngFunc_SetSize, g_Oberon, Float:{-40.0, -40.0, -40.0}, Float:{40.0, 40.0, 96.0})
	engfunc(EngFunc_SetOrigin, g_Oberon, Origin)
	
	static Float:Angles[3]
	pev(g_Oberon, pev_angles, Angles)
	
	Angles[1] -= 85
	
	set_pev(g_Oberon, pev_angles, Angles)
...
public Think_Boss(iActivityEnt) {
	const iBoss = get_entvar(iActivityEnt, var_owner);
			...
			switch(pev(Ent, pev_num)) {
				case 0: {
					...
					if(Agr) {
						...
						Anim(Ent, randoms ? 9 : 10, 1.0)
						set_pev(Ent, pev_nextthink, get_gametime() + (randoms ? 0.6 : 0.3))
					} else {
						Sound(Ent, randoms ? 2 : 3)
						Anim(Ent, randoms ? 2 : 3, 1.0)
						set_pev(Ent, pev_nextthink, get_gametime() + (randoms ? 1.15 : 0.55))
					}
					return
				}
				case 1: {
			...
			switch (pev(Ent, pev_num)) {
				...
				case 1:
				{
					...
					for(new id = 1; id <= get_maxplayers(); id++)
					{
						...
						Len = Move(id, Ent, 700.0, Velocity, Angle)
		...
		case JUMP: {
			...
			switch (pev(Ent, pev_num)) {
				case 0: {
					...
					Move(Ent, JumpTarget, 800.0, Velocity, Angle)
					set_pev(Ent, pev_angles, Angle)				

					set_pev(Ent, pev_nextthink, get_gametime() + 0.5)
					set_pev(Ent, pev_num, 1)
					Anim(Ent, Agr ? 11 : 4, 1.0)
				}
				case 1: {					
					Sound(Ent, 9)
					Velocity[2] = 1000.0
...
public MapEvent() {
	e_boss = engfunc(EngFunc_CreateNamedEntity, engfunc(EngFunc_AllocString, "info_target"))
