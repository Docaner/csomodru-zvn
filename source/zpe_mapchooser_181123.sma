...
#define TIME_TOVOTE 20 // Время на голосование
...
new Array:g_asMap, Array:g_aiMinOnline, Array:g_aiMaxOnline,
...
prepare_to_vote()
{
	...
	set_pcvar_float(g_pCvarTimeLimit, fTimeLimit + fRoundTime);

	g_fOldFreezeTime = get_pcvar_float(g_pCvarFreezeTime);
	g_bFreezeTime = true;

	set_pcvar_float(g_pCvarFreezeTime, float(TIME_TOVOTE) + 6.0);
...
public ClCmd_RTV(id)
{
	...
	if(!g_bIsRTV)
	{
		UTIL_SayText(id, "!g[ZP|MAP] !yВозможность проголосовать будет доступна через !t%d !yс", 
			floatround(g_flTimeRTV - get_gametime(), floatround_ceil));
		return;
	}
...
load_maps()
{
	...
	new szBuffer[512], iLine, iLen, 
		szMap[32], szMinOnline[4], szMaxOnline[4], iMin, iMax;
	while(read_file(g_szFile, iLine++, szBuffer, charsmax(szBuffer), iLen))
	{
		...
		parse(szBuffer, szMap, charsmax(szMap), szMinOnline, charsmax(szMinOnline), szMaxOnline, charsmax(szMaxOnline))
