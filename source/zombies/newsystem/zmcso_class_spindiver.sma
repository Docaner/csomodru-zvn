...
#define ZM_CLASS_POUNCE_POWER 	486
#define ZM_CLASS_COUNTDOWN		22.0
...
public plugin_init( ) 
{
	...
	g_iszSpinDiver = zc_find_zclass_by_shortname("spin");
...
{
	if(zp_get_user_zombie(iPlayer) && zc_get_user_zclass(iPlayer) == g_iszSpinDiver && !zp_get_user_nemesis(iPlayer))
	{
...
public zp_round_ended(winteam)
{
	...
	public CWeapon__Deploy_Post(iItem)
	{
		...
		if(zc_get_user_zclass(iPlayer) != g_iszSpinDiver) return HAM_IGNORED;
...
public Command_Ability(iPlayer)
{
	...
	if(zc_get_user_zclass(iPlayer) == g_iszSpinDiver)
	{
		//static Flags; 
		new Float: g_flVel[3];
...
public FM_Hook_PlayerPreThink_Pre(iPlayer)
{
	...
	if(zc_get_user_zclass(iPlayer) != g_iszSpinDiver) return FMRES_IGNORED;
