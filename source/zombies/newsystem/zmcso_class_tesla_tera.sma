...
public plugin_init()
{
	...
	g_iClassTesla = zc_find_zclass_by_shortname("tesla");
...
{
	if(is_user_alive(id) && !zp_get_user_nemesis(id) && zp_get_user_zombie(id) && zc_get_user_zclass(id) == g_iClassTesla)
...
{
	if(!zp_get_user_nemesis(id) && zp_get_user_zombie(id) && zc_get_user_zclass(id) == g_iClassTesla)
		set_default_values(id);
}
...
{
	if(zc_get_user_zclass(id) == g_iClassTesla)
	{
		if(nemesis)
			set_default_values(id);
		else
	...
	if(is_user_alive(infector) && zc_get_user_zclass(infector) == g_iClassTesla && !zp_get_user_nemesis(infector) && !zp_is_survivor_round())
	{
...
public zp_round_ended(winteam)
{
	for(new i = 1; i <= get_maxplayers(); i++)
	{
		...
		if(!zp_get_user_nemesis(i) && zp_get_user_zombie(i) && zc_get_user_zclass(i) == g_iClassTesla)
...
{
	if(is_user_alive(iAttacker) && zp_get_user_zombie(iAttacker) && zc_get_user_zclass(iAttacker) == g_iClassTesla && 
	...
	if(!zp_get_user_nemesis(iVictim) && zp_get_user_zombie(iVictim) && zc_get_user_zclass(iVictim) == g_iClassTesla)
...
public fw_UpdateClientData_Post(id, SendWeapons, CD_Handle) 
{
	...
	if(!is_user_alive(id) || !zp_get_user_zombie(id) || zp_get_user_nemesis(id) ||
		zc_get_user_zclass(id) != g_iClassTesla) 
		return;
...
public HM_KnifeDeploy_Post(iEnt)
{
	...
	if(!zp_get_user_zombie(id) || zp_get_user_nemesis(id) ||
		zc_get_user_zclass(id) != g_iClassTesla) return HAM_IGNORED;
