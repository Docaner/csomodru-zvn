...
public plugin_init()
{
	...
	g_iClassShaman = zc_find_zclass_by_shortname("shaman");
...
{
	if(is_user_alive(id) && !zp_get_user_nemesis(id) && zp_get_user_zombie(id) && zc_get_user_zclass(id) == g_iClassShaman)
		set_default_values(id);
}

public zp_user_humanized_pre(id, survivor)
{
	if(!zp_get_user_nemesis(id) && zp_get_user_zombie(id) && zc_get_user_zclass(id) == g_iClassShaman)
		set_default_values(id);
}

public zp_user_infected_post(id, infector, nemesis)
{
	if(zc_get_user_zclass(id) == g_iClassShaman)
	{
		if(nemesis)
			set_default_values(id);
		else
...
public zp_round_ended(winteam)
{
	...
	{
		if(is_user_alive(i) && !zp_get_user_nemesis(i) && zp_get_user_zombie(i) && zc_get_user_zclass(i) == g_iClassShaman)
...
{
	if(!zp_get_user_nemesis(iVictim) && zp_get_user_zombie(iVictim) && zc_get_user_zclass(iVictim) == g_iClassShaman)
...
public HM_KnifeDeploy_Post(iEnt)
{
	...
	if(!zp_get_user_zombie(id) || zp_get_user_nemesis(id) || 
		zc_get_user_zclass(id) != g_iClassShaman) 
		return HAM_IGNORED;
