...
//Уникальный ключ
new const g_qstKey[] =          "Usas";
//Название
new const g_qstName[] =         "Барабан 12к = мясорубка!"; 
//Описание
new const g_qstDesc[] =         "Убейте определёное количество^nзомби с Usas-12"
//Категория
...
public plugin_init()
{
    register_plugin("[QST] Usas", "1.0", "Docaner");
...
@RG__PlayerKilled_Post(pVictim, pAttacker, iGibs)
{
    ...
    if( (pActiveItem = get_member(pAttacker, m_pActiveItem)) == NULLENT || get_entvar(pActiveItem, var_impulse) != IMPULSE_Usas)
        return;
