...
//Уникальный ключ
new const g_qstKey[] =          "M777";
//Название
new const g_qstName[] =         "Лего настолько опасно?!"; 
//Описание
new const g_qstDesc[] =         "Убейте определёное количество^nзомби с M777 Shotgun"
//Категория
...
public plugin_init()
{
    register_plugin("[QST] M777", "1.0", "Docaner");
...
@RG__PlayerKilled_Post(pVictim, pAttacker, iGibs)
{
    ...
    if( (pActiveItem = get_member(pAttacker, m_pActiveItem)) == NULLENT || get_entvar(pActiveItem, var_impulse) != IMPULSE_M777)
        return;
