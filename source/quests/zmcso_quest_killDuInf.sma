...
//Уникальный ключ
new const g_qstKey[] =          "DualInf";
//Название
new const g_qstName[] =         "Akimbo time!"; 
//Описание
new const g_qstDesc[] =         "Убейте определёное количество^nзомби с Dual Infinity"
//Категория
...
public plugin_init()
{
    register_plugin("[QST] DualInf", "1.0", "Docaner");
...
@RG__PlayerKilled_Post(pVictim, pAttacker, iGibs)
{
    ...
    if( (pActiveItem = get_member(pAttacker, m_pActiveItem)) == NULLENT || get_entvar(pActiveItem, var_impulse) != IMPULSE_DuIn)
        return;
