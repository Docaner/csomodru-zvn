...
//Уникальный ключ
new const g_qstKey[] =          "killshaman";
//Название
new const g_qstName[] =         "Пора закрыть ему рот"; 
//Описание
new const g_qstDesc[] =         "Убейте определёное количество^nзомби-шаманов"
//Категория
...
//Название класса
new const g_szShamanName[] = "ML_SHAMAN_NAME"

new g_iQst;

new g_iZombie_Shaman;

public plugin_init()
{
    register_plugin("[QST] Kill Shaman", "1.0", "Docaner");
    ...
    g_iZombie_Shaman = zp_get_zombie_class_id(g_szShamanName);

    
    //console_print(0, "CLASS: %d [%s]", g_iZombie_Shaman, g_szShamanName);
}

@RG__PlayerKilled_Post(pVictim, pAttacker, iGibs)
{
    if(!zp_get_user_zombie(pVictim) || zp_get_user_zombie_class(pVictim) != g_iZombie_Shaman)
        return;
