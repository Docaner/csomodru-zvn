...
//Уникальный ключ
new const g_qstKey[] =          "killdoctor";
//Название
new const g_qstName[] =         "Последнее исцеление"; 
//Описание
new const g_qstDesc[] =         "Убейте определёное количество^nзомби-докторов"
//Категория
...
//Название класса
new const g_szHealerName[] = "ML_HEALER_NAME"

new g_iQst;

new g_iZombie_Healer;

public plugin_init()
{
    register_plugin("[QST] Kill Healer", "1.0", "Docaner");
    ...
    g_iZombie_Healer = zp_get_zombie_class_id(g_szHealerName);

    
    //console_print(0, "CLASS: %d [%s]", g_iZombie_Healer, g_szHealerName);
}

@RG__PlayerKilled_Post(pVictim, pAttacker, iGibs)
{
    if(!zp_get_user_zombie(pVictim) || zp_get_user_zombie_class(pVictim) != g_iZombie_Healer)
        return;
