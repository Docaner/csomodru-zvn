...
//Уникальный ключ
new const g_qstKey[] =          "QuadBar";
//Название
new const g_qstName[] =         "Вот это бум-палка!"; 
//Описание
new const g_qstDesc[] =         "Убейте определёное количество^nзомби с Quad Barrel"
//Категория
...
public plugin_init()
{
    register_plugin("[QST] QuadBar", "1.0", "Docaner");
...
{
    if(!zp_get_user_zombie(pVictim) || !(get_member(pVictim, m_bitsDamageType) & (DMG_BULLET)))
        return;
    ...
    if( (pActiveItem = get_member(pAttacker, m_pActiveItem)) == NULLENT || get_entvar(pActiveItem, var_impulse) != IMPULSE_QuBr)
        return;
