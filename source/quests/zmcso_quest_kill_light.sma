...
//Уникальный ключ
new const g_qstKey[] =          "killlight";
//Название
new const g_qstName[] =         "Покойся с миром, малая..."; 
//Описание
new const g_qstDesc[] =         "Убейте определёное количество^nлегких зомби"
//Категория
...
//Название класса
new const g_szLightName[] = "ML_SPEED_NAME"
...
public plugin_init()
{
    register_plugin("[QST] Kill Light", "1.0", "Docaner");
    ...
    g_iZombie_Light = zp_get_zombie_class_id(g_szLightName);

    
    //console_print(0, "CLASS: %d [%s]", g_iZombie_Light, g_szLightName);
}

@RG__PlayerKilled_Post(pVictim, pAttacker, iGibs)
{
    if(!zp_get_user_zombie(pVictim) || zp_get_user_zombie_class(pVictim) != g_iZombie_Light)
        return;
